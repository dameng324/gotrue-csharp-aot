...
namespace Supabase.Gotrue
{
    ...
    public class Api : IGotrueApi<User, Session>
    {
        ...
        /// <returns></returns>
        public async Task<Session?> SignUpWithEmail(
            string email,
            string password,
        ...
        )
        {
            ...
            var response = await Helpers.MakeRequest2(HttpMethod.Post, endpoint, body, Headers);
            ...
            if (!string.IsNullOrEmpty(response.Content))
            {
                ...
                var session = JsonSerializer.Deserialize<Session>(
                    response.Content!,
                    GotrueJsonContext.Default.Session
                );
                ...
                if (session is { User: null })
                {
                    ...
                    session.User = JsonSerializer.Deserialize<User>(
                        response.Content!,
                        GotrueJsonContext.Default.User
                    );
                }
        ...
        public Task<Session?> SignInWithEmail(string email, string password)
        {
            ...
            return Helpers.MakeRequest2<Session>(
                HttpMethod.Post,
                $"{Url}/token?grant_type=password",
                jsonTypeInfo: GotrueJsonContext.Default.Session,
                data: body,
                headers: Headers
            );
        }
        ...
        /// <returns></returns>
        public async Task<PasswordlessSignInState> SignInWithOtp(
            SignInWithPasswordlessEmailOptions options
        )
        {
            var url = string.IsNullOrEmpty(options.EmailRedirectTo)
                ? $"{Url}/otp"
                : $"{Url}/otp?redirect_to={options.EmailRedirectTo}";
            ...
            if (!string.IsNullOrEmpty(options.CaptchaToken))
                body.Add(
                    ...
                    new Dictionary<string, string> { { "captcha_token", options.CaptchaToken! } }
                );

            await Helpers.MakeRequest2(HttpMethod.Post, url, body, Headers);
        ...
        /// <returns></returns>
        public async Task<PasswordlessSignInState> SignInWithOtp(
            SignInWithPasswordlessPhoneOptions options
        )
        {
            var url = $"{Url}/otp";
        ...
        public async Task<Session?> SignInAnonymously(SignInAnonymouslyOptions? options = null)
        {
            ...
            return await Helpers.MakeRequest2<Session>(
                HttpMethod.Post,
                url,
                jsonTypeInfo: GotrueJsonContext.Default.Session,
                data: body,
        ...
        /// </exception>
        public Task<Session?> SignInWithIdToken(
            Provider provider,
            string idToken,
        ...
        )
        {
            if (
                ...
                throw new GotrueException(
                    $"Provider must be `Google`, `Apple`, `Azure`, or `Facebook` not {provider}"
                );

            var body = new Dictionary<string, object>
            {
                { "provider", provider.GetMappedString() },
                { "id_token", idToken },
            };
            ...
            if (!string.IsNullOrEmpty(captchaToken))
                body.Add(
                    "gotrue_meta_security",
                    new Dictionary<string, object?> { { "captcha_token", captchaToken } }
                );

            return Helpers.MakeRequest2<Session>(
                HttpMethod.Post,
                $"{Url}/token?grant_type=id_token",
                jsonTypeInfo: GotrueJsonContext.Default.Session,
                data: body,
                headers: Headers
            );
        ...
        private Task<SSOResponse?> SignInWithSsoInternal(
            Guid? providerId = null,
            string? domain = null,
            SignInWithSSOOptions? options = null
        )
        {
            if (providerId != null && domain != null)
                throw new GotrueException(
                    $"Both providerId and domain were provided to the API, "
                        + $"you must supply either one or the other but not both providerId={providerId}, domain={domain}"
                );
            if (providerId == null && domain == null)
                throw new GotrueException(
                    $"Both providerId and domain were null "
                        + $"you must supply either one or the other but not both providerId={providerId}, domain={domain}"
                );
            ...
            else
            {
                body["domain"] =
                    domain
                    ?? throw new GotrueException(
                        "Domain must be provided if providerId is not provided"
                    );
            }

            if (!string.IsNullOrWhiteSpace(options?.RedirectTo))
            ...
            if (!string.IsNullOrEmpty(options?.CaptchaToken))
                body.Add(
                    "gotrue_meta_security",
                    new Dictionary<string, object?> { { "captcha_token", options?.CaptchaToken } }
                );

            return Helpers.MakeRequest2<SSOResponse>(
                HttpMethod.Post,
                $"{Url}/sso",
                jsonTypeInfo: GotrueJsonContext.Default.SSOResponse,
                data: body,
                headers: Headers
            );
        }
        ...
        /// <inheritdoc />
        public Task<SSOResponse?> SignInWithSSO(
            Guid providerId,
            SignInWithSSOOptions? options = null
        )
        {
        ...
        public Task<BaseResponse> SendMagicLinkEmail(string email, SignInOptions? options = null)
        {
            ...
            return Helpers.MakeRequest2(HttpMethod.Post, endpoint, data, Headers);
        }
        ...
        /// <param name="options"></param>
        /// <returns></returns>
        public Task<BaseResponse> InviteUserByEmail(
            string email,
            string jwt,
        ...
        {
            var url =
                options == null || string.IsNullOrEmpty(options.RedirectTo)
                    ? $"{Url}/invite"
                    : $"{Url}/invite?redirect_to={options.RedirectTo}";
            var body = new Dictionary<string, object> { { "email", email } };

            if (options?.Data != null)
                body["data"] = options.Data;

            return Helpers.MakeRequest2(
                HttpMethod.Post,
                url,
                body,
        ...
        /// <param name="options">Optional Signup data.</param>
        /// <returns></returns>
        public Task<Session?> SignUpWithPhone(
            string phone,
            string password,
            SignUpOptions? options = null
        )
        ...
        public Task<BaseResponse> SendMobileOTP(string phone)
        {
            ...
            return Helpers.MakeRequest2(HttpMethod.Post, $"{Url}/otp", data, Headers);
        }
        ...
        {
            var data = new Dictionary<string, string> { { "email", email } };
            return Helpers.MakeRequest2(HttpMethod.Post, $"{Url}/recover", data, Headers);
        }
        ...
        /// <param name="options"></param>
        /// <returns></returns>
        public async Task<ResetPasswordForEmailState> ResetPasswordForEmail(
            ResetPasswordForEmailOptions options
        )
        {
            var url = string.IsNullOrEmpty(options.RedirectTo)
                ? $"{Url}/recover"
                : $"{Url}/recover?redirect_to={options.RedirectTo}";
            string? verifier = null;

            var body = new Dictionary<string, object> { { "email", options.Email } };
        ...
        /// <inheritdoc />
        public ProviderAuthState GetUriForProvider(
            Provider provider,
        ...
        public Task<MfaEnrollResponse?> Enroll(string jwt, MfaEnrollParams mfaEnrollParams)
        {
            ...
            if (!string.IsNullOrWhiteSpace(mfaEnrollParams.FriendlyName))
                body["friendly_name"] = mfaEnrollParams.FriendlyName;
            if (!string.IsNullOrWhiteSpace(mfaEnrollParams.Issuer))
                body["issuer"] = mfaEnrollParams.Issuer;

            return Helpers.MakeRequest2<MfaEnrollResponse>(
                HttpMethod.Post,
                $"{Url}/factors",
                jsonTypeInfo: GotrueJsonContext.Default.MfaEnrollResponse,
                data: body,
                headers: CreateAuthedRequestHeaders(jwt)
            );
        }

        /// <inheritdoc />
        public Task<MfaChallengeResponse?> Challenge(
            string jwt,
            MfaChallengeParams mfaChallengeParams
        )
        {
            return Helpers.MakeRequest2<MfaChallengeResponse>(
                HttpMethod.Post,
                $"{Url}/factors/{mfaChallengeParams.FactorId}/challenge",
                jsonTypeInfo: GotrueJsonContext.Default.MfaChallengeResponse,
                headers: CreateAuthedRequestHeaders(jwt)
            );
        }
        ...
        public Task<MfaVerifyResponse?> Verify(string jwt, MfaVerifyParams mfaVerifyParams)
        {
            ...
            return Helpers.MakeRequest2<MfaVerifyResponse>(
                HttpMethod.Post,
                $"{Url}/factors/{mfaVerifyParams.FactorId}/verify",
                jsonTypeInfo: GotrueJsonContext.Default.MfaVerifyResponse,
                data: body,
                headers: CreateAuthedRequestHeaders(jwt)
            );
        }
        ...
        {
            return Helpers.MakeRequest2<MfaUnenrollResponse>(
                HttpMethod.Delete,
                $"{Url}/factors/{mfaUnenrollParams.FactorId}",
                GotrueJsonContext.Default.MfaUnenrollResponse,
                CreateAuthedRequestHeaders(jwt)
            );
        }

        /// <inheritdoc />
        public Task<BaseResponse> ListFactors(
            string jwt,
        ...
        )
        {
            return Helpers.MakeRequest2(
                HttpMethod.Get,
                $"{Url}/admin/users/{listFactorsParams.UserId}/factors",
                CreateAuthedRequestHeaders(jwt)
            );
        }
        ...
        {
            return Helpers.MakeRequest2<MfaAdminDeleteFactorResponse>(
                HttpMethod.Delete,
                $"{Url}/admin/users/{deleteFactorParams.UserId}/factors/{deleteFactorParams.Id}",
                GotrueJsonContext.Default.MfaAdminDeleteFactorResponse,
                CreateAuthedRequestHeaders(jwt)
            );
        }

        /// <inheritdoc />
        public async Task<ProviderAuthState> LinkIdentity(
            string token,
            Provider provider,
            SignInOptions options
        )
        {
            var state = Helpers.GetUrlForProvider(
                $"{Url}/user/identities/authorize",
                provider,
                options
            );
            await Helpers.MakeRequest2(
                HttpMethod.Get,
                state.Uri.ToString(),
                CreateAuthedRequestHeaders(token)
            );
            return state;
        }
        ...
        {
            var result = await Helpers.MakeRequest2(
                HttpMethod.Delete,
                $"{Url}/user/identities/${userIdentity.IdentityId}",
                CreateAuthedRequestHeaders(token)
            );
        ...
        public Task<BaseResponse> SignOut(string jwt, SignOutScope scope = SignOutScope.Global)
        {
            ...
            return Helpers.MakeRequest2(
                HttpMethod.Post,
                $"{Url}/logout?scope={scope.GetMappedString()}",
                data,
                CreateAuthedRequestHeaders(jwt)
            );
        }
        ...
        public Task<User?> GetUser(string jwt)
        {
            ...
            return Helpers.MakeRequest2<User>(
                HttpMethod.Get,
                $"{Url}/user",
                jsonTypeInfo: GotrueJsonContext.Default.User,
                data: data,
                headers: CreateAuthedRequestHeaders(jwt)
            );
        }
        ...
        )
        {
            ...
            return Helpers.MakeRequest2<UserList<User>>(
                HttpMethod.Get,
                $"{Url}/admin/users",
                jsonTypeInfo: GotrueJsonContext.Default.UserListUser,
                data: data,
                headers: CreateAuthedRequestHeaders(jwt)
            );
        }

        private Dictionary<string, string> TransformListUsersParams(
            string? filter = null,
            string? sortBy = null,
            SortOrder sortOrder = SortOrder.Descending,
            int? page = null,
        ...
        )
        {
            ...
            {
                var mapTo = sortOrder.GetMappedString();
                query.Add("sort", $"{sortBy} {mapTo}");
            }
        ...
        public Task<BaseResponse> Reauthenticate(string userJwt)
        {
            return Helpers.MakeRequest2(
                HttpMethod.Get,
                $"{Url}/reauthenticate",
                CreateAuthedRequestHeaders(userJwt)
            );
        }
        ...
        {
            return Helpers.MakeRequest2<Settings>(
                HttpMethod.Get,
                $"{Url}/settings",
                jsonTypeInfo: GotrueJsonContext.Default.Settings,
                Headers
            );
        }
        ...
        public Task<BaseResponse> GenerateLink(string jwt, GenerateLinkOptions options)
        {
            var url = string.IsNullOrEmpty(options.RedirectTo)
                ...
                : $"{Url}/admin/generate_link?redirect_to={options.RedirectTo}";

            return Helpers.MakeRequest(
                HttpMethod.Post,
                url,
                GotrueJsonContext.Default.GenerateLinkOptions,
                options,
                CreateAuthedRequestHeaders(jwt)
            );
        }
        ...
        public Task<Session?> RefreshAccessToken(string accessToken, string refreshToken)
        {
            ...
            var data = new Dictionary<string, string> { { "refresh_token", refreshToken } };
            ...
            return Helpers.MakeRequest2<Session>(
                ...
                jsonTypeInfo: GotrueJsonContext.Default.Session,
                data: data,
                headers: Headers.MergeLeft(headers)
            );
        }
    }
}
