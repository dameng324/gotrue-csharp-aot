...
namespace Supabase.Gotrue
{
    ...
    public class Client : IGotrueClient<User, Session>
    {
        ...
        public void AddStateChangedListener(
            IGotrueClient<User, Session>.AuthEventHandler authEventHandler
        )
        {
        ...
        /// <inheritdoc />
        public async Task<Session?> SignInWithIdToken(
            Provider provider,
            string idToken,
        ...
        )
        {
            ...
            var result = await _api.SignInWithIdToken(
                provider,
                idToken,
                accessToken,
                nonce,
        ...
        public Task<Session?> SignIn(string email, string password) =>
            SignIn(SignInType.Email, email, password);
        ...
        )
        {
            ...
            if (
                newSession?.User?.ConfirmedAt == null
        ...
        public Task<ProviderAuthState> LinkIdentity(Provider provider, SignInOptions options)
        {
            ...
            if (options.FlowType != OAuthFlowType.PKCE)
                throw new GotrueException(
                    "PKCE flow type is required for this action.",
                    InvalidFlowType
                );
        ...
        /// <inheritdoc />
        public async Task<Session> SetSession(
            string accessToken,
            string refreshToken,
        ...
        )
        {
            ...
            if (string.IsNullOrEmpty(accessToken) || string.IsNullOrEmpty(refreshToken))
                throw new GotrueException(
                    "`accessToken` and `refreshToken` cannot be empty.",
                    NoSessionFound
                );
            ...
            if (payload == null || payload.ValidTo == DateTime.MinValue)
                throw new GotrueException(
                    "`accessToken`'s payload was of an unknown structure.",
                    NoSessionFound
                );
            ...
            if (payload.ValidTo < DateTime.UtcNow || forceAccessTokenRefresh)
            {
                ...
                if (result == null || string.IsNullOrEmpty(result.AccessToken))
                    throw new GotrueException(
                        "Could not generate a session given the provided parameters.",
                        NoSessionFound
                    );
        ...
        public async Task RefreshToken(string accessToken, string refreshToken)
        {
            ...
            try
            {
                ...
                if (result == null || string.IsNullOrEmpty(result.AccessToken))
                    throw new GotrueException(
                        "Could not refresh token from provided session.",
                        NoSessionFound
                    );
        ...
        public async Task RefreshToken()
        {
            ...
            if (
                ...
                || string.IsNullOrEmpty(CurrentSession?.RefreshToken)
            )
                throw new GotrueException("No current session.", NoSessionFound);

            try
            {
                var result = await _api.RefreshAccessToken(
        ...
        /// <inheritdoc />
        public async Task<Session?> ChallengeAndVerify(
        ...
        )
        {
            ...
            var challengeResponse = await _api.Challenge(
                CurrentSession.AccessToken,
        ...
        public Task<MfaListFactorsResponse> ListFactors()
        {
            ...
            var response = new MfaListFactorsResponse()
            {
                ...
                Totp = CurrentSession
                    .User!.Factors.Where(x => x.FactorType == "totp" && x.Status == "verified")
                    .ToList(),
            };
        ...
        public Task<MfaGetAuthenticatorAssuranceLevelResponse> GetAuthenticatorAssuranceLevel()
        {
            ...
            var payload = new JwtSecurityTokenHandler()
                .ReadJwtToken(CurrentSession.AccessToken)
                .Payload;
            ...
            var verifiedFactors =
                CurrentSession.User!.Factors?.Where(factor => factor.Status == "verified").ToList()
                ?? new List<Factor>();
            ...
            var currentAuthenticationMethods = payload.Amr.Select(x =>
