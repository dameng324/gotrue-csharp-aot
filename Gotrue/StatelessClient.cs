...
namespace Supabase.Gotrue
{
    ...
    public class StatelessClient : IGotrueStatelessClient<User, Session>
    {
        ...
        /// <inheritdoc />
        public async Task<MfaEnrollResponse?> Enroll(
            string jwt,
        ...
        /// <inheritdoc />
        public async Task<MfaChallengeResponse?> Challenge(
            string jwt,
        ...
        /// <inheritdoc />
        public async Task<MfaVerifyResponse?> Verify(
            string jwt,
        ...
        /// <inheritdoc />
        public async Task<MfaVerifyResponse?> ChallengeAndVerify(
            string jwt,
        ...
        {
            var api = GetApi(options);
            var challengeResponse = await api.Challenge(
                jwt,
                new MfaChallengeParams { FactorId = mfaChallengeAndVerifyParams.FactorId }
            );
        ...
        /// <inheritdoc />
        public async Task<MfaUnenrollResponse?> Unenroll(
            string jwt,
        ...
        /// <inheritdoc />
        public async Task<MfaListFactorsResponse?> ListFactors(
            string jwt,
            StatelessClientOptions options
        )
        {
            ...
            if (user != null)
            {
                var response = new MfaListFactorsResponse()
                {
                    All = user.Factors,
                    Totp = user
                        .Factors.Where(x => x.FactorType == "totp" && x.Status == "verified")
                        .ToList(),
                };
        ...
        /// <summary>
        /// Gets the current and next authenticator assurance level for a user based on their enrolled factors and the "aal" claim in their JWT.
        /// </summary>
        /// <param name="jwt"></param>
        /// <param name="options"></param>
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        public async Task<MfaGetAuthenticatorAssuranceLevelResponse?> GetAuthenticatorAssuranceLevel(
            string jwt,
            StatelessClientOptions options
        )
        {
            ...
            if (user != null)
            {
                ...
                if (payload.TryGetValue("aal", out var aal))
                {
                    currentLevel = Enum.TryParse(
                        aal.ToString(),
                        out AuthenticatorAssuranceLevel parsedLevel
                    )
                        ? parsedLevel
                ...
                var verifiedFactors =
                    user.Factors?.Where(factor => factor.Status == "verified").ToList()
                    ?? new List<Factor>();
                ...
                var currentAuthenticationMethods = payload
                    .Amr.Select(x =>
                        JsonSerializer.Deserialize<AmrEntry>(x, GotrueJsonContext.Default.AmrEntry)
                    )
        ...
        public IGotrueApi<User, Session> GetApi(StatelessClientOptions options) =>
            new Api(options.Url, options.Headers);

        /// <inheritdoc />
        public Task<Session?> SignUp(
            string email,
            string password,
            StatelessClientOptions options,
            SignUpOptions? signUpOptions = null
        ) => SignUp(SignUpType.Email, email, password, options, signUpOptions);

        /// <inheritdoc />
        public async Task<Session?> SignUp(
            SignUpType type,
            string identifier,
            string password,
            StatelessClientOptions options,
        ...
        )
        {
            ...
            if (
                session?.User?.ConfirmedAt != null
                || session?.User != null && options.AllowUnconfirmedUserSessions
            )
            {
                return session;
            }
        ...
        /// <inheritdoc />
        public Task<bool> SendMagicLink(
            string email,
            StatelessClientOptions options,
            SignInOptions? signInOptions = null
        ) => SignIn(email, options, signInOptions);

        /// <inheritdoc />
        public Task<Session?> SignIn(
            string email,
            string password,
            StatelessClientOptions options
        ) => SignIn(SignInType.Email, email, password, options);

        /// <inheritdoc />
        public async Task<Session?> SignIn(
            SignInType type,
        ...
        /// <inheritdoc />
        public ProviderAuthState SignIn(
            Provider provider,
            StatelessClientOptions options,
            SignInOptions? signInOptions = null
        ) => GetApi(options).GetUriForProvider(provider, signInOptions);
        ...
        /// <inheritdoc />
        public async Task<Session?> VerifyOTP(
            string phone,
            string otpToken,
            StatelessClientOptions options,
        ...
        /// <inheritdoc />
        public async Task<Session?> VerifyTokenHash(
            string tokenHash,
            StatelessClientOptions options,
        ...
        public async Task<bool> InviteUserByEmail(
            ...
            StatelessClientOptions options,
            InviteUserByEmailOptions? invitationOptions = null
        )
        {
            var response = await GetApi(options)
                .InviteUserByEmail(email, serviceRoleToken, invitationOptions);
        ...
        /// <inheritdoc />
        public async Task<UserList<User>?> ListUsers(
            string serviceRoleToken,
            StatelessClientOptions options,
            string? filter = null,
            string? sortBy = null,
        ...
        {
            return await GetApi(options)
                .ListUsers(serviceRoleToken, filter, sortBy, sortOrder, page, perPage);
        }
        ...
        /// <inheritdoc />
        public async Task<Session?> ExchangeCodeForSession(
            string codeVerifier,
            string authCode,
            StatelessClientOptions options
        )
