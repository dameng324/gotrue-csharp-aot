...
namespace GotrueTests
{
    ...
    public class AnonKeyClientTests
    {
        ...
        public async Task SaveAndLoadUser()
        {
            ...
            IGotrueClient<User, Session> newClient = new Client(
        ...
        public async Task SignUpUserPhone()
        {
            ...
            var session = await _client.SignUp(
            ...
            AreEqual("Testing", session.User.UserMetadata["firstName"].GetString());
        }
        ...
        public async Task ClientTriggersTokenRefreshedEvent()
        {
            ...
            _client.AddStateChangedListener(
                (_, args) =>
                {
        ...
        public async Task ClientSignsIn()
        {
            ...
            var newSession = await _client.SignIn(
                Constants.SignInType.RefreshToken,
                refreshToken ?? throw new InvalidOperationException()
            );
            AreEqual(_client.CurrentSession, _persistence.SavedSession);
            Contains(_stateChanges, TokenRefreshed);
        ...
        public async Task ClientSignsInAnonymously()
        {
            ...
            AreEqual("John", session.User.UserMetadata["first_name"].GetString());
        }
        ...
        public async Task ClientSendsMagicLoginEmailAlias()
        {
            ...
            var result2 = await _client.SendMagicLink(
                user2,
        ...
        {
            var result = await _client.SignIn(
        ...
        public async Task ClientGetUser()
        {
            ...
            var userByJWT = await _client.GetUser(
                newUser.AccessToken ?? throw new InvalidOperationException()
            );
        ...
        public async Task ClientLinkIdentityPKCE()
        {
            ...
            await ThrowsExceptionAsync<GotrueException>(async () =>
                await _client.LinkIdentity(Constants.Provider.Github, new SignInOptions())
            );

            var session = await _client.SignUp(email, PASSWORD);

            var result = await _client.LinkIdentity(
                Constants.Provider.Github,
                new SignInOptions { FlowType = Constants.OAuthFlowType.PKCE }
            );
        ...
        public async Task ClientCanChangePassword()
        {
            ...
            await _client.Update(new UserAttributes() { Password = newPassword });
