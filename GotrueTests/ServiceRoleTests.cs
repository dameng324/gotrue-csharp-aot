    ...
    public class ServiceRoleTests
    {
        ...
        public void TestInitializer()
        {
            _client = new AdminClient(
                _serviceKey,
                new ClientOptions { AllowUnconfirmedUserSessions = true }
            );
        }
        ...
        public async Task UpdateUser()
        {
            ...
            var result2 = await _client.UpdateUserById(
                session.Id,
                new AdminUserAttributes
                {
                    UserMetadata = new Dictionary<string, JsonElement>
                    {
                        { "hello", "updated".ToJsonElement() },
                    },
                }
            );

            AreEqual("updated", result2.UserMetadata["hello"].GetString());

            var result3 = await _client.GetUserById(session.Id);
            AreEqual("updated", result3.UserMetadata["hello"].GetString());
        }
        ...
        {
            var result1 = await _client.ListUsers(
                sortBy: "created_at",
                sortOrder: SortOrder.Ascending
            );
            var result2 = await _client.ListUsers(
                sortBy: "created_at",
        ...
        public async Task GetUserById()
        {
            ...
            var userByIdResult = await _client.GetUserById(
                userResult.Id ?? throw new InvalidOperationException()
            );
        ...
        public async Task CreateUser()
        {
            ...
            var attributes = new AdminUserAttributes
            {
                UserMetadata = new Dictionary<string, JsonElement>
                {
                    { "firstName", "123".ToJsonElement() },
                },
                AppMetadata = new Dictionary<string, JsonElement>
                {
                    {
                        "roles",
                        new List<string> { "editor", "publisher" }.ToJsonElement(
                            TestJsonContext.Default.ListString
                        )
                    },
                },
            };

            var result2 = await _client.CreateUser(
                $"{RandomString(12)}@supabase.io",
                PASSWORD,
                attributes
            );
            AreEqual("123", result2.UserMetadata["firstName"].GetString());

            var result3 = await _client.CreateUser(
                new AdminUserAttributes
                {
                    Email = $"{RandomString(12)}@supabase.io",
                    Password = PASSWORD,
                }
            );
            IsNotNull(result3);
        ...
        public async Task UpdateUserById()
        {
            ...
            var updatedUser = await _client.UpdateUserById(
                createdUser.Id ?? throw new InvalidOperationException(),
        ...
        public async Task BanUserById()
        {
            ...
            var updatedUser = await _client.UpdateUserById(
                createdUser.Id ?? throw new InvalidOperationException(),
                new AdminUserAttributes { BanDuration = $"{banDurationSeconds}s" }
            );
            ...
            updatedUser = await _client.UpdateUserById(
                createdUser.Id ?? throw new InvalidOperationException(),
                new AdminUserAttributes { BanDuration = "none" }
            );
            IsNotNull(updatedUser);
        ...
        public void NonceGeneration()
        {
            ...
            AreEqual(
                "f533268b824e95839010fd876b7f565e3f69c9220b1a701f42ccfec97c2cd321",
                helloAppleVerifier
            );
        }
