...
namespace GotrueTests
{
    ...
    public class StatelessClientTests
    {
        ...
        {
            get => new StatelessClientOptions() { AllowUnconfirmedUserSessions = true };
        }
        ...
        private string GenerateServiceRoleToken()
        {
            var signingKey = new SymmetricSecurityKey(
                Encoding.UTF8.GetBytes("37c304f8-51aa-419a-a1af-06154e63707a")
            ); // using GOTRUE_JWT_SECRET
        ...
        public async Task SignsUpUser()
        {
            ...
            Assert.IsNotNull(session.AccessToken);
            Assert.AreEqual("Testing", session.User.UserMetadata["firstName"].GetString());
        }
        ...
        public async Task SignsIn()
        {
            ...
            // Refresh Token
            var newSession = await _client.RefreshToken(
                session.AccessToken,
        ...
        public async Task SendsMagicLoginEmail()
        {
            ...
            var result2 = await _client.SignIn(
                user2,
                Options,
        ...
        public void ReturnsAuthUrlForProvider()
        {
            ...
            Assert.AreEqual(
                ...
                result1.Uri.ToString()
            );

            var result2 = _client.SignIn(
                Provider.Google,
                Options,
                new SignInOptions { Scopes = "special scopes please" }
            );
            Assert.AreEqual(
                "http://localhost:9999/authorize?provider=google&scopes=special+scopes+please",
        ...
        {
            var serviceRoleKey = GenerateServiceRoleToken();
            var result = await _client.CreateUser(
                serviceRoleKey,
                Options,
            ...
            Assert.AreEqual("123", result2.UserMetadata["firstName"].GetString());
        ...
        {
            var serviceRoleKey = GenerateServiceRoleToken();
            var createdUser = await _client.CreateUser(
                serviceRoleKey,
                Options,
                $"{RandomString(12)}@supabase.io",
                PASSWORD
        ...
        public async Task MfaEnroll()
        {
            ...
            var enrollResponse = await _client.Enroll(
                session.AccessToken,
                new MfaEnrollParams
                {
                    FactorType = "totp",
            ...
            var challengeAndVerify = await _client.ChallengeAndVerify(
                session.AccessToken,
            ...
            assuranceLevel = await _client.GetAuthenticatorAssuranceLevel(
                challengeAndVerify.AccessToken,
                Options
            );
